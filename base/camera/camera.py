...
class Cam():
    ...
    def show(self, start_time):
        ...
        if cfg["camera"].get("record", True):
            if self._video_writer is None:
                videos_dir = Path.cwd() / "videos"
                videos_dir.mkdir(exist_ok=True)
                model_name = Path(cfg["inference"]["new_model"]).stem
                self._video_file = str(
                    videos_dir / f"output_{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.avi"
                )
                height, width = frame.shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*'XVID')
                self._video_writer = cv2.VideoWriter(
                    self._video_file, fourcc, cfg["camera"]["fps"], (width, height)
                )
                print(f"[INFO] Video recording started: {self._video_file}")
    ...
    def release(self):
        self._stop_record.value = 1
        if self._video_writer is not None:
            self._video_writer.release()
            print(f"[INFO] Video saved: {self._video_file}")
